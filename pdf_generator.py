...
def draw_header(c, report_date, generated_display_time, market_as_of_date=None, page=1, total=2, report_status="PASS"):
    ...
    NAVY_LEFT = colors.HexColor("#082E5B")
    BLUE_MAIN = colors.HexColor("#1F5FA3")
    CYAN_LINE = colors.HexColor("#12B5EA")
    DARK_SEPARATOR = colors.HexColor("#050505")
    GOLD_LINE = colors.HexColor("#C7A44C")
    ...
    fr(c, 0, H - HEADER_H, LEFT_W, HEADER_H, NAVY_LEFT)
    fr(c, LEFT_W, H - HEADER_H, W - LEFT_W, HEADER_H, BLUE_MAIN)

    # ------------------------------------------------------------
    # Vertical divider
    # ------------------------------------------------------------

    c.setStrokeColor(colors.HexColor("#3E7CC0"))
    c.setLineWidth(0.5)
    c.line(LEFT_W, H - HEADER_H + 2 * mm, LEFT_W, H - 2 * mm)
